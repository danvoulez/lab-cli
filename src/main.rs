...
fn emit_legacy_act(
    ...
    if (200..300).contains(&code) {
        ...
        return register_fallback(
            ...
            key,
            LEDGER,
            &serde_json::to_string(&Value::Object(raw)).unwrap_or_default(),
        );
    }
    resp
}
...
fn register_fallback(url: &str, key: &str, table_attempted: &str, raw: &str) -> String {
    ...
    if (200..300).contains(&code) {
        let tier = if table_attempted == LEDGER {
            ...
            format!(
                "raw · wrapped into LogLine receipt (from {})",
                table_attempted
            )
        };
...
fn print_tail(url: &str, key: &str, n: usize) {
    let q = format!("select=act,inserted_at&order=inserted_at.desc&limit={}", n);
...
fn mark_seen(state: &mut serde_json::Map<String, Value>, key: &str, instant: &clock::UtcInstant) {
    state.insert(key.into(), Value::String(instant.to_rfc3339()));
}
...
fn queue_act(
    ...
    extra.insert(
        "target_process".into(),
        Value::String(target_process.into()),
    );
    extra.insert(
        "playbook_macro".into(),
        Value::String(playbook_macro.into()),
    );
    extra.insert(
...
fn cmd_executor_run(url: &str, key: &str, box_id: &str) -> bool {
    ...
    if lab_home().join("PAUSE").exists() {
        eprintln!(
            "lab executor: PAUSE file exists; not running {}. Remove ~/.lab/PAUSE to resume.",
            original_hash
        );
        return false;
    }
    ...
    eprintln!(
        "lab executor: running {} with playbook {} (estimated {} ACU)",
        original_hash, playbook_macro, needed_acu
    );
    record_acu_spent(needed_acu);
    let status = Command::new("devin").args(["-p", &prompt]).status();
    match status {
        Ok(s) if s.success() => {
            let _ = fs::remove_file(&path);
...
fn budget_to_value(b: &Budget) -> Value {
    ...
    m.insert(
        "daily_acu_max".into(),
        Value::Number(serde_json::Number::from_f64(b.daily_acu_max).unwrap_or_else(|| 0.into())),
    );
    m.insert(
        "reset_hour_utc".into(),
        Value::Number(b.reset_hour_utc.into()),
    );
    m.insert(
        "spent_today".into(),
        Value::Number(serde_json::Number::from_f64(b.spent_today).unwrap_or_else(|| 0.into())),
    );
    ...
    match cmd {
        ...
        "schedule" => {
            ...
            if clock::UtcInstant::parse(&at).is_err() {
                eprintln!(
                    "lab schedule: invalid time '{}' (expected RFC3339, e.g. 2026-06-21T12:00:00Z)",
                    at
                );
                exit(2);
            }
            ...
            if !(200..300).contains(&code) {
                eprintln!(
                    "lab schedule: ledger rejected (HTTP {}): {}",
                    code,
                    resp.trim()
                );
                exit(1);
            ...
            eprintln!(
                "   scheduled → logline_acts [canonical] {} at {}",
                c_hash, at
            );
            print!("{}", resp);
        }
